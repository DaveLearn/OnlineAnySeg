...
#
# DEG wrapper around scripts/mask_predict/mask_predict_single_seq_w_semantic.py:
# path-injects the bootstrapped Entity/CropFormer tree (no "copy scripts into
# detectron2" step) and fixes the upstream crash on frames with zero valid masks.
# Runs in the `cropformer` pixi environment.
import argparse
import glob
...
def configure_cropformer_imports() -> Path:
    ...
    # helpers.py (crop/pad/pallete utilities) lives next to the upstream script
    sys.path.insert(1, str(Path(__file__).resolve().parent / 'scripts' / 'mask_predict'))
...
from helpers import get_new_pallete, get_cropped_image, pad_into_square
...
def get_parser():
    parser = argparse.ArgumentParser(description="CropFormer + CLIP per-mask embeddings for OnlineAnySeg")
    parser.add_argument(
        "--cropformer-root",
        type=Path,
    ...
    parser.add_argument(
        "--config-file",
        default=str(CROPFORMER_ROOT / "configs/entityv2/entity_segmentation/mask2former_hornet_3x.yaml"),
        metavar="FILE",
    ...
    parser.add_argument(
        "--pretrained_path",
        type=str,
        default="checkpoints/clip/open_clip_pytorch_model.bin",
    )
    parser.add_argument(
        "--dst_h",
        type=int,
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 2.1: select by confidence threshold (upstream read the global `args` here)
...
if __name__ == "__main__":
    ...
    argv = sys.argv[1:]
    if argv[:1] == ["--"]:  # pixi run task -- <args> passes the separator through
        argv = argv[1:]
    ...
    for i, (path, img, selected_scores, selected_masks) in tqdm(enumerate(my_dataloader)):
        ...
        if selected_masks.shape[0] == 0:
            # upstream crashed here (torch.stack on an empty list). Writing neither the mask
            # png nor the embeddings file makes the dataset loader treat this frame as
            # unsegmented (seg_flag=False), which is the correct semantics.
            print("frame_%d: 0 valid instances, skipping mask/embedding export..." % i)
            continue
        ...
        if valid_instance_num == 0:
            # every mask fell below min_mask_pixel_size -- same unsegmented-frame semantics
            print("frame_%d: all masks below min_mask_pixel_size, skipping export..." % i)
            continue
